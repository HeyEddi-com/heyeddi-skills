...
def test_publish_ci_pack_materialize(tmp_path: Path) -> None:
    ...
    pack = json.loads((ROOT / "packs" / "heyeddi-ci-skills.json").read_text())
    ...
    reg = json.loads((out / "skills-registry.json").read_text())
    assert reg["version"] == pack["version"]
    assert "HeyEddi-com/heyeddi-ci-skills" in (out / "README.md").read_text()
