...
4. **Plan gate:** `python .agents/skills/engineering-excellence/scripts/check_engineering_plan.py --project-root <root> --check`
5. Read `reference/material-handoff.md` + Implementation spec.
6. Update `lib/theme/app_theme.dart` from spec.
7. Build `lib/widgets/app_shell.dart` (drawer + scaffold).
8. `python scripts/verify_handoff.py --route <route> --phase shell --check`
9. `python scripts/verify_tokens.py --check` + `python scripts/verify_theme.py --check`
10. Build `lib/screens/<feature>_screen.dart`.
11. `python scripts/verify_handoff.py --route <route> --phase full --check`
12. **Change gate:** `python .agents/skills/engineering-excellence/scripts/audit_engineering.py --project-root <root> --check`
13. Chain `@visual-auditor` with `FLUTTER_WEB_URL=http://127.0.0.1:8085`
