...
1. **Session start** (or first HeyEddi task in a thin workspace): sync / skills index current; know which `@skill` owns the work. If `.heyeddi/stack.json` is missing required prefs, route **`@heyeddi-setup`** first.
